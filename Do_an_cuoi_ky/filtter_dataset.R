...
#Tần số tối đa cộng với 8% cho ylim trong biểu đồ
...
sidebar <- shinydashboardPlus::dashboardSidebar(
  id = 'msidebarid',
  # Tháo phần tử chuyển đổi thanh bên
...
#################################################################################
# Bảng điều khiển thân và thanh bên là một phần của UI
#################################################################################
body <- dashboardBody(
  useShinyalert(),
  shinyjs::useShinyjs(),
...
#################################################################################
# Thiết lập chức năng lớp server
#################################################################################

server <- function(input, output, session) {
  ##Dây là để ẩn thanh bên phải
  ...
  #################################################################################
  # Bắt đầu chức năng upload dataset
  #################################################################################
        ...
        column(
          ...
          column(
            ...
            fluidRow(
              ...
              # Input: Chọn dấu phân cách kiểu file ----
  ...
  observeEvent(input$file,{
    ...
    if (ext == "xls" | ext == 'xlsx'){
      vmy$mydata <- as.data.frame(readxl::read_excel(input$file$datapath))  
    }
    else if (ext == "csv"){
      tryCatch({            #tránh lỗi hiển thị rscript ban đầu trước khi thực hiện
        ...
        # trả về safeError nếu xảy ra lỗi phân tích cú pháp
        stop(safeError(e))})
    }else
    {
  ...
  #################################################################################
  # Mã tải lên tập dữ liệu tệp ENDS tại đây
  #################################################################################
  ...
  #################################################################################
  # Tập dữ liệu tệp mã MODIFY bắt đầu từ đây
  #################################################################################
  ...
  #################################################################################
  # Tập dữ liệu tệp mã DELETE VARIABLE bắt đầu từ đây
  #################################################################################
  
  ### chọn cột để xoá
  ### Tạo lời nhắn trước khi xoá
  ...
  ### Nếu người dùng nhấn OK, sau đó chọn hàng đề xoá
  ...
  #################################################################################
  # Tệp dữ liệu mã DELETE VARIABLE ENDS tại đây
  #################################################################################
  ...
  #######- trên nhiều bảng kết thúc dữ liệu
  ...
  output$muimultisliderplay <- renderUI({
    tryCatch({
      ...
      # Đầu tiên, tạo một danh sách các thanh trượt với mỗi tên khác nhau
      ...
      # Tạo một tagList của các thanh trượt (điều này rất quan trọng)
  ...
  vmy$data_1 <-reactive({
    tryCatch({ 
      ...
      # đây là cách lấy các biến đầu vào từ ui
      ...
      slider_optionsTXT <-  input$mChrVarFilter 
      # đây là cách tìm nạp các biến đầu vào từ các trường ký tự thành phần ui
  ...
  # đường màu đỏ là mean và màu đen là trung bình
